...
mutable struct CCBarrier
    ...
    function CCBarrier()
        ...
        worker_sense =
            SharedArray{Bool}((nworkers(),), init = (sa) -> fill!(sa, false))
...
mutable struct CCReducer{FT}
    ...
    function CCReducer{FT}() where {FT}
        ...
        worker_sense =
            SharedArray{Bool}((nworkers(),), init = (sa) -> fill!(sa, false))
        vals =
            SharedArray{FT}((nworkers(),), init = (sa) -> fill!(sa, zero(FT)))
...
struct SACommsContext <: ClimaComms.AbstractCommsContext
    ...
    function SACommsContext(neighbors::Vector{ClimaComms.Neighbor{B}}) where {B}
...
get_stage(sa::SharedArray) = sdata(sa)
...
function ClimaComms.barrier(ctx::SACommsContext)
    ...
    if ClimaComms.iamroot(ctx)
        for pid in workers()
            while barr.worker_sense[pid] == barr.sense
                yield()
            end
        end
        barr.sense = !barr.sense
    else
        barr.worker_sense[ClimaComms.mypid(ctx)] =
            !barr.worker_sense[ClimaComms.mypid(ctx)]
        while barr.sense != barr.worker_sense[ClimaComms.mypid(ctx)]
            yield()
        end
    end
end
...
function ClimaComms.reduce(ctx::SACommsContext, val, op)
    ...
    if ClimaComms.iamroot(ctx)
        for i in 1:nworkers()
            while reducer.worker_sense[pid] == reducer.sense
                yield()
            end
            val = op(val, reducer.vals[i])
        end
        reducer.sense = !reducer.sense
        return val
    else
        reducer.vals[ClimaComms.mypid(ctx)] = val
        reducer.worker_sense[ClimaComms.mypid(ctx)] =
            !reducer.worker_sense[ClimaComms.mypid(ctx)]
        while reducer.sense != reducer.worker_sense[ClimaComms.mypid(ctx)]
            yield()
        end
    end
end
